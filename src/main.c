...
static void sort_util(struct graph *graph, struct graph_node **visited,
                      ...
                      u32 *stack_count, struct graph_node *node)
{
        ...
        for (u32 o = 0; o < node->output_edges_count; ++o) {
                ...
                sort_util(graph, visited, visited_count, stack, stack_count,
                          child_node);
        }
...
static void graph_topological_sort(struct graph *graph)
{
        ...
        for (u32 i = 0; i < graph->nodes_count; i++) {
                ...
                sort_util(graph, visited_nodes, &visited_count, node_stack,
                          &stack_size, node);
        }
                             ...
                             b8 trans_b, f32 alpha, f32 beta)
{
        ...
        ASSERT(bias.dim_count == 1 && bias.dim[0] == (trans_b ? b.dim[0] : b.dim[1]));
        if (!trans_a && !trans_b) {
